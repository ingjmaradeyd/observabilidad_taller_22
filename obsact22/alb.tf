...
resource "aws_lb_target_group" "service_a" {
  name        = substr("${local.name}-a-tg", 0, 32)
  ...
  health_check {
    enabled             = true
    path                = var.service_a_health_path
    protocol            = "HTTP"
    matcher             = "200-399"
    interval            = 30
    timeout             = 5
...
resource "aws_lb_target_group" "service_b" {
  name        = substr("${local.name}-b-tg", 0, 32)
  ...
  health_check {
    enabled             = true
    path                = var.service_b_health_path
    protocol            = "HTTP"
    matcher             = "200-399"
    interval            = 30
    timeout             = 5
